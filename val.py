...
run = wandb.init(project="nacc-temporal",
                 entity="jemoka", config=CONFIG, mode="disabled")
...
model.load_state_dict(torch.load(os.path.join(config.load, "model.save")))
...
torch.save(l_l, f"./results/{Path.stem}")
